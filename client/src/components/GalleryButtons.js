...
import { SHOW_FOLDER, CONFIRM_DELETE, CONFIRM_DELETE_BATCH, IMG2IMG, MIRROR, SEND_TO_IMG2IMG, SHOW_BATCH, EDITING, IMG2VID } from '../constants/features';
...
export const SendToImgButt = ({ data }) => {
    ...
    const sendImgToImg = (location) => {
        // instead of disabling button, simply block the callback
    ...
    const menuItemsData = { label: '', items: [{ label: 'Send to Img2Img', callback: () => sendImgToImg(IMG2IMG) }, { label: 'Send to Img2Vid', callback: () => sendImgToImg(IMG2VID) }, { label: 'Send to Editing', callback: () => sendImgToImg(EDITING) }] }
    return (
        <NestedDropdown
